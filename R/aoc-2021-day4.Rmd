...
# check_colmatch <- function(df, char1){
# }
...
for(i in bingo_choices){
  ...
  if(nrow(solution_test_a)>1){
    used_choices <- append(used_choices, i)
    print(paste0("winning choice: ", i))
    print(solution_test_a)
    solution_board <- solution_test_a

    win_group_number <- solution_board %>% select(group1) %>% unique() %>% pull() %>% str_remove("G") %>% as.numeric()
    print(win_group_number)
    data1_steps <- data1_steps[-win_group_number]
    solution_boards <- c(solution_boards, solution_test_a)
    # break
  }else if(nrow(solution_test_b)>1){
    used_choices <- append(used_choices, i)
    print(paste0("winning choice: ", i))
    ...
    win_group_number <- solution_board %>% select(group1) %>% unique() %>% pull() %>% str_remove("G") %>% as.numeric()
    print(win_group_number)
    data1_steps_transp <- data1_steps_transp[-win_group_number]
    solution_boards <- c(solution_boards, solution_test_b)
    # break
  }
}
# solution_board <- solution_boards %>% tail(1)
last_choice <- used_choices %>% tail(1) %>% as.numeric()
sum_unmarked_numbs <- solution_board %>% select(-group1) %>%
